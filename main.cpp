  ...
  for (int i = 0; i < table_count; i++) {
    ...
    if (table.is_available() && !table.is_reserved()) {
      ...
      if (size == request_size || size == request_size + 1) {
        cout << "Table " << i + 1 << " is currently availble." << endl;
        return;
      }
    }
  }
...
// An utility function to read input from user to select table number;
...
// Reads the membership datafile and returns the count of membership data
// stored.
int get_membership_data_count() {
  ifstream fin;
  int count = 0;
  fin.open(MEMBERSHIP_DATA_FILENAME.c_str());
  string line;
...
int main() {
  ...
  cin >> table_count;
  membership_data_count = get_membership_data_count();
  Table *table_pool = new Table[table_count];
  Member *membership_data = new Member[membership_data_count];
